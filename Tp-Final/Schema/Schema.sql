...
CREATE PROCEDURE Company_Add(IN jobPositionId INTEGER,IN companyName VARCHAR(60),IN description VARCHAR(60))
BEGIN

	INSERT INTO Company(jobPositionId,companyName,description) VALUES (jobPositionId,companyName,description);
...
BEGIN
	SELECT companyId,jobPositionId,companyName,description
    FROM Company;
END$$
...
CREATE PROCEDURE Company_Update(IN company_id INT,jobPositionId INT,IN companyName VARCHAR(50),IN description VARCHAR(200))
BEGIN
		UPDATE Company set jobPositionId = jobPositionId ,companyName = companyName,description = description WHERE companyId = company_id ;
